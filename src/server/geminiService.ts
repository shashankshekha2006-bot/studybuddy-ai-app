...
export async function askQuestion(params: AskParams, apiKey?: string) {
  const { question, subject = 'General', classLevel = 10, language = 'English', explanationLevel = 'Simple' } = params;

  if (!question || typeof question !== 'string' || question.trim().length === 0) {
    throw new Error('Please enter a valid school question. (कृपया एक प्रश्न पूछें)');
  }
  ...
  if (!ai) {
    // Offline/Fallback pedagogical response
    return {
      finalAnswer: `Core Concept for Class ${classLevel} (${subject}): Remember to state standard textbook definitions and units.`,
      simpleExplanation: `For Class ${classLevel} students learning in ${language}, this topic revolves around foundational NCERT concepts. We break down the question into simple relatable components with real-world examples.`,
      stepByStepSolution: [
        `Step 1: Identify given information and the exact variable or concept requested in the question.`,
        `Step 2: State the appropriate NCERT formula, theorem, or standard definition for Class ${classLevel}.`,
...
`;
  ...
  return await generateWithFallback(ai, { prompt, schema });
}
...
export async function generateQuiz(params: QuizParams, apiKey?: string) {
  const { classLevel = 10, subject = 'General', topic, numQuestions = 5, language = 'English' } = params;

  if (!topic || typeof topic !== 'string' || topic.trim().length === 0) {
    throw new Error('Please provide a topic or chapter name for the quiz.');
  }
...
export async function generateNotes(params: NotesParams, apiKey?: string) {
  const { classLevel = 10, subject = 'General', topic, language = 'English' } = params;

  if (!topic || typeof topic !== 'string' || topic.trim().length === 0) {
    throw new Error('Please enter a topic or chapter name to generate notes.');
  }
